...
function isInsideCircle(circle, point) {
  const kat1 = point.x - circle.center.x;
  const kat2 = point.y - circle.center.y;
  const hypo = Math.sqrt((kat1 ** 2) + (kat2 ** 2));
  if (hypo < circle.radius) {
    return true;
  }
  return false;
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (!uni.includes(char) && !arr.includes(char)) {
      return char;
    }
    uni.push(char);
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes[0].length; i += 1) {
    ...
    if (isCommon === true) {
      ...
      result.reverse().splice(0, result.indexOf('/'));
      result.reverse();
      return result.join('');
    }
  }
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < res.length; i += 1) {
    res[i] = new Array(m2[i].length).fill(0);
...
function evaluateTicTacToePosition(position) {
  ...
  if (position[2][2]) {
    if ((position[2][2] === position[0][2] && position[2][2] === position[1][2])
